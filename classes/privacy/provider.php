...
/**
 * Privacy Subsystem implementation for paygw_payeer.
 *
...
namespace paygw_payeer\privacy;
...
class provider implements \core_privacy\local\request\data_provider, paygw_provider, \core_privacy\local\metadata\provider {
    ...
    public static function get_metadata(collection $collection): collection {
        ...
        $collection->add_external_location_link(
            'payeer.plus',
            [
                'shopid'   => 'privacy:metadata:paygw_payeer:shopid',
                'apikey'   => 'privacy:metadata:paygw_payeer:apikey',
                'email'    => 'privacy:metadata:paygw_payeer:email',
            ],
            'privacy:metadata:paygw_payeer:payeer_plus'
        );

        // The paygw_payeer has a database table that contains user data.
        $collection->add_database_table(
            'paygw_payeer',
            [
                'invoiceid'  => 'privacy:metadata:paygw_payeer:invoiceid',
                'courceid'   => 'privacy:metadata:paygw_payeer:courceid',
                'groupnames' => 'privacy:metadata:paygw_payeer:groupnames',
                'success'    => 'privacy:metadata:paygw_payeer:success',
            ],
            'privacy:metadata:paygw_payeer:paygw_payeer'
        );
        return $collection;
    }
    ...
    public static function export_payment_data(\context $context, array $subcontext, \stdClass $payment) {
        ...
        $subcontext[] = get_string('gatewayname', 'paygw_payeer');
        $record = $DB->get_record('paygw_payeer', ['paymentid' => $payment->id]);
    ...
    public static function delete_data_for_payment_sql(string $paymentsql, array $paymentparams) {
        ...
        $DB->delete_records_select('paygw_payeer', "paymentid IN ({$paymentsql})", $paymentparams);
    }
}
