...
$surcharge = helper::get_gateway_surcharge('payeer');// In case user uses surcharge.
...
if (!$transactionid = $DB->insert_record('paygw_payeer', $paygwdata)) {
    die(get_string('error_txdatabase', 'paygw_payeer'));
}
...
if (!empty($password) || $skipmode) {
    ...
    if ($success) {
        ...
        redirect($url, get_string('password_success', 'paygw_payeer'), 0, 'success');
    } else {
        redirect($url, get_string('password_error', 'paygw_payeer'), 0, 'error');
    }
    die; // Never.
}
