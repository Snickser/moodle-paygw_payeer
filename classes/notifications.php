...
/** Notifications for paygw_payeer.
 *
...
class notifications {
    /**
     * Function that handle the notifications about transactions using payeer payment gateway
    ...
    public static function notify($userid, $fee, $currency, $orderid, $type = '') {
        ...
        $message->component = 'paygw_payeer';
        $message->name      = 'payment_receipt'; // The notification name from message.php.
        $message->userfrom  = \core_user::get_noreply_user(); // If the message is 'from' a specific user you can set them here.
        $message->userto    = $user;
        $message->subject   = get_string('messagesubject', 'paygw_payeer', $type);
        switch ($type) {
            case 'Success completed':
                $messagebody = get_string('message_success_completed', 'paygw_payeer', $a);
                break;
        }
