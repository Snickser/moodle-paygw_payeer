...
if (!$payeertx = $DB->get_record('paygw_payeer', ['paymentid' => $id])) {
...
if ($payeertx->success) {
    redirect($url, get_string('payment_success', 'paygw_payeer'), 0, 'success');
} else {
    redirect($url, get_string('payment_error', 'paygw_payeer'), 0, 'error');
}
